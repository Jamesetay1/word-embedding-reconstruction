...
def set_result_dest(args):
    ...
    if args.network_type == 0:
        ...
        print('error')
        exit()
    result_dest = args.result_dir+f"/{type_name}/{args.n_min}-{args.n_max}_"+start_time
    return result_dest
...
def main(args):
    ...
    print("JT: Error is from data_processor.prepare_dataset()")
    n_vocab_subword = data_processor.n_vocab_subword
    print("JT: Data setup done")

    # model setup
    model, optimizer = model_setup(args, n_vocab_subword)

    print("JT: model setup done")
    ...
    print("JT: iterator/update/trainer setup done")
